...
def create_types_xml(loot_entries):
    ...
    # Spawn rate and rarity multipliers
    tier_multipliers = {
        "green": 15,    # Common pistols
        "blue": 10,     # SMGs
        "yellow": 7,    # Mixed items
        "purple": 5,    # Assault rifles
        "red": 2        # Sniper rifles and high-end weapons
    }

    for entry in loot_entries:
        weapon_class = entry['dna_TheChosenOne']
        tier = entry['dna_Tier']
        
        type_elem = ET.SubElement(root, "type", name=weapon_class)
        
        # Nominal based on tier
        ...
        # Restock
        ET.SubElement(type_elem, "restock").text = "0"
